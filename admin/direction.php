...
{
   $id= $_GET['del'];
   $msg     = $objgen->del_Row("direction","id=".$id);
    if($msg=="")
   {
...
if($_GET['msg']==2)
{
  ...
  $msg2 = "Direction Updated Successfully.";
}

if($_GET['msg']==3)
{

  $msg2 = "Direction Deleted Successfully.";
}
...
{
  $un = trim($_REQUEST['un']);
  $where .= " and direction_name like '%".$un."%'";
}



$row_count = $objgen->get_AllRowscnt("direction",$where);
if($row_count>0)
{
  ...
  $res_arr = $objgen->get_AllRows("direction",$pagesize*($page-1),$pagesize,"id desc",$where);
}
            ...
            <div class="panel-body">
              <form class="form-inline" method="post" enctype="multipart/form-data" >
			    <div class="form-group">
                  <label class="form-label" for="example1">Direction Name</label>
                                     ...
                                     <table id="example1" class="table table-bordered table-striped">
										...
										if($row_count>0)
										{
										  foreach($res_arr as $key=>$val)
											...
											?>
                                            <tr>
                                                <td><?php echo $objgen->check_tag($val['direction_name']); ?></td>
                                                <td><?php echo $objgen->basedecode($val['direction']); ?></td>
                                                <td><a href="<?=$add_url?>/?edit=<?=$val['id']?>&page=<?=$page?>" role="button" ><span class="fa fa-edit"></span></a></td>
                                                <td><a href="<?=$list_url?>/?del=<?=$val['id']?>&page=<?=$page?>" role="button" onClick="return confirm('Do you want to delete this Direction?')"><span class="fa fa-trash-o"></span></a></td>
                                            </tr>
