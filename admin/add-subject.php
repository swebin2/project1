...
$pagehead = "Subject";
$list_url = URLAD."subject";
$add_url  = URLAD."add-subject";

$page	 	= isset($_REQUEST['page'])	?	$_REQUEST['page']	:	"1";

if($_GET['msg']==1)
{
  $msg2 = "Subject Created Successfully.";
}
...
if(isset($_POST['Create']))
{
   ...
   $rules		=	array();
   $rules[] 	= "required,name,Enter the Subject";
   $errors  	= $objval->validateFields($_POST, $rules);
   
   $brd_exit = $objgen->chk_Ext("subject","name='$name' and exam_id=$exam_id");
	if($brd_exit>0)
	{
		$errors[] = "This subject is already exists.";
	}

   if(empty($errors))
	{
		 
		 $msg = $objgen->ins_Row('subject','name,exam_id,code',"'".$name."','".$exam_id."','".$code."'");
		 if($msg=="")
		 {
...
if(isset($_GET['edit']))
{
       ...
       $id = $_GET['edit'];
	   $result   = $objgen->get_Onerow("subject","AND id=".$id);
...
{    
   ...
   $brd_exit = $objgen->chk_Ext("subject","name='$name' and exam_id=$exam_id and id<>$id");
	if($brd_exit>0)
	{
		$errors[] = "This subject is already exists.";
	}
   ...
   if(empty($errors))
	{
	  ...
	  $msg = $objgen->upd_Row('subject',"name='".$name."',exam_id='".$exam_id."',code='".$code."'","id=".$id);
	  if($msg=="")
	  {
